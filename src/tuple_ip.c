/*
 * Sweetspot -- an OSI layer 3 network access controller.
 * Written by Ilya Etingof <ilya@glas.net>, 2006.
 *
 * Partially based on http://chillispot.org.
 * Copyright (C) 2003, 2004, 2005 Mondru AB.
 * The contents of this file may be used under the terms of the GNU
 * General Public License Version 2, provided that the above copyright
 * notice and this permission notice is included in all copies or
 * substantial portions of the software.
 */

#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
#include <stdio.h>
#include <ctype.h>
#include <string.h>
#include <stdlib.h>
#include <unistd.h>
#if defined(HAVE_STDINT_H)
#include <stdint.h>
#elif defined(HAVE_INTTYPES_H)
#include <inttypes.h>
#endif
#include <sys/types.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <netinet/ip.h>
#include <arpa/inet.h>
#include <errno.h>
#include "sweetspot.h"
#include "fnv.h"
#include "pretty.h"
#include "tuple.h"
#include "tuple_ip.h"
#include "tuple_checksum.h"
#include "log.h"
#ifdef USE_DMALLOC
#include <dmalloc.h>
#endif

static int __fetch(sw_tuple_t *tuple_chain_a, sw_tuple_t *tuple_chain_b,
                   char *packet, int length) {
  sw_tuple_ip_t *prv_a, *prv_b;
  struct iphdr *iph = (struct iphdr *) packet;

  if (iph->version != 4) { /* not IP */
    if (sw_debug_flags & SW_DEBUG_TUPLE)
      sw_log("%s:%d not IPv4 packet (%d)", __FILE__, __LINE__, iph->version);
    return -1;
  }

  if (length < sizeof(struct iphdr) || length < ntohs(iph->tot_len)) {
    if (sw_debug_flags & SW_DEBUG_TUPLE)
      sw_log("%s:%d short IP packet %d < %d|%d", __FILE__, __LINE__,
             length, ntohs(iph->tot_len), sizeof(struct iphdr));
    return -1;
  }

#ifdef USE_MEMMGR
    sw_memgmt_tuple_buf_t * buf;
    if(sw_memgmt_tuple_get_buf(&buf,sizeof(sw_tuple_ip_t)) == -1) return -1;
    prv_a = (sw_tuple_ip_t *)&buf->buf;
    prv_a->mem_idx = buf->idx;
#else
    prv_a = (sw_tuple_ip_t *)malloc(sizeof(sw_tuple_ip_t));

  if (!prv_a) {
    sw_log("%s:%d malloc() failed: %s", __FILE__, __LINE__, strerror(errno));
    return -1;
  }
#endif
#ifdef USE_MEMMGR
    if(sw_memgmt_tuple_get_buf(&buf,sizeof(sw_tuple_ip_t)) == -1) return -1;
    prv_b = (sw_tuple_ip_t *)&buf->buf;
    prv_b->mem_idx = buf->idx;
#else
    prv_b = (sw_tuple_ip_t *)malloc(sizeof(sw_tuple_ip_t));

  if (!prv_b) {
    sw_log("%s:%d malloc() failed: %s", __FILE__, __LINE__, strerror(errno));
    free(prv_a);
    return -1;
  }
#endif



  prv_a->ip = iph->saddr;
  prv_a->id = iph->id;
  prv_a->frag_off = iph->frag_off;



  prv_b->ip = iph->daddr;
  prv_b->id = iph->id;
  prv_b->frag_off = iph->frag_off;

  tuple_chain_a->prv = prv_a;
  tuple_chain_b->prv = prv_b;

  if (sw_debug_flags & SW_DEBUG_TUPLE)
    sw_log("%s:%d got IP %s->%s %s", __FILE__, __LINE__,
           sw_pretty_ip(ntohl(prv_a->ip)), sw_pretty_ip(ntohl(prv_b->ip)),
           ntohs(prv_a->frag_off) & (IP_OFFMASK | IP_MF) ? "(fragment)" : "");

  /* In case of a fragment, further parse only the first fragment */
  if (ntohs(prv_a->frag_off) & IP_OFFMASK)
    return 0;

  return sw_tuple_fetch(&(tuple_chain_a->next),
                        &(tuple_chain_b->next),
                        iph->protocol,
                        packet+iph->ihl*4,
                        ntohs(iph->tot_len)-iph->ihl*4);
}

static int __commit(char *packet, int length,
                    sw_tuple_t *tuple_chain_a,
                    sw_tuple_t *tuple_chain_b,
                    sw_tuple_t *tuple_chain_org_a,
                    sw_tuple_t *tuple_chain_org_b) {
  sw_tuple_ip_t *prv_a = tuple_chain_a->prv, *prv_b = tuple_chain_b->prv;
  struct iphdr *iph = (struct iphdr *) packet;
  uint16_t cs, partial_cs;
  int rc;

  if (length < sizeof(struct iphdr) || length < ntohs(iph->tot_len)) {
    sw_log("%s:%d short IP packet %d < %d|%d", __FILE__, __LINE__, 
           length, ntohs(iph->tot_len), sizeof(struct iphdr));
    return -1;
  }

  if (sw_tuple_checksum_decrement(&partial_cs,
                                  ntohs(iph->check), 1,
                                  (unsigned char *)&(iph->saddr),
                                  sizeof(iph->saddr) + 
                                  sizeof(iph->daddr)) == -1) {
    return -1;
  }

  rc = sw_tuple_commit(packet+iph->ihl*4,
                       ntohs(iph->tot_len)-iph->ihl*4,
                       tuple_chain_a->next,
                       tuple_chain_b->next,
                       tuple_chain_org_a->next,
                       tuple_chain_org_b->next);

  iph->saddr = prv_a->ip;
  iph->daddr = prv_b->ip;

  if (sw_debug_flags & SW_DEBUG_TUPLE)
    sw_log("%s:%d IP set to %s->%s", __FILE__, __LINE__,
           sw_pretty_ip(ntohl(prv_a->ip)), 
           sw_pretty_ip(ntohl(prv_b->ip)));
  
  if (sw_tuple_checksum_increment(&cs, partial_cs, 1,
                                  (unsigned char *)&(iph->saddr),
                                  sizeof(iph->saddr) +
                                  sizeof(iph->daddr)) == -1) {
    return -1;
  }

  iph->check = htons(cs);

  return rc;
}

static uint32_t __hash(sw_tuple_t *tuple_chain) {
  sw_tuple_ip_t *prv = tuple_chain->prv;
  struct _h {
    uint16_t protocol;
    uint16_t id;
    uint16_t frag_off;
    uint32_t ip;
  } h;
  memset(&h, 0, sizeof(h));
  h.protocol = tuple_chain->protocol;
  h.id = prv->id;
  h.frag_off = prv->frag_off;
  h.ip = prv->ip;
  return fnv_32a_buf((char *)&h, sizeof(h), sw_tuple_hash(tuple_chain->next));
}

static uint32_t __hash_simpl__(sw_tuple_t *tuple_chain) {
  sw_tuple_ip_t *prv = tuple_chain->prv;
  struct _h {
    uint16_t protocol;
    uint16_t id;
    uint16_t frag_off;
    uint32_t ip;
  } h;
  memset(&h, 0, sizeof(h));
  h.protocol = tuple_chain->protocol;
  h.frag_off = prv->frag_off;
  h.ip = prv->ip;
  return fnv_32a_buf((char *)&h, sizeof(h), sw_tuple_hash_simpl(tuple_chain->next));
}

static int __cmp(sw_tuple_t *tuple_chain_a, sw_tuple_t *tuple_chain_b) {
  sw_tuple_ip_t *prv_a = tuple_chain_a->prv;
  sw_tuple_ip_t *prv_b = tuple_chain_b->prv;
  if (prv_a->ip != prv_b->ip || \
      prv_a->id != prv_b->id || \
      prv_a->frag_off != prv_b->frag_off) {
    if (sw_debug_flags & SW_DEBUG_TUPLE)
      sw_log("%s:%d IPs differ %s != %s", __FILE__, __LINE__,
             sw_pretty_ip(ntohl(prv_a->ip)), sw_pretty_ip(ntohl(prv_b->ip)));
    return 1;
  }
  return sw_tuple_cmp(tuple_chain_a->next, tuple_chain_b->next);
}

static int __clone(sw_tuple_t *new_tuple_chain, sw_tuple_t *tuple_chain) {
#ifdef USE_MEMMGR
    sw_memgmt_tuple_buf_t * buf;
    if(sw_memgmt_tuple_get_buf(&buf,sizeof(sw_tuple_ip_t)) == -1) return -1;
    new_tuple_chain->prv = (sw_tuple_ip_t *)&buf->buf;
    sw_tuple_ip_t * tmp = (sw_tuple_ip_t *)new_tuple_chain->prv;
    memcpy(new_tuple_chain->prv, tuple_chain->prv, sizeof(sw_tuple_ip_t));
    tmp->mem_idx = buf->idx;
#else

  new_tuple_chain->prv = (sw_tuple_ip_t *)malloc(sizeof(sw_tuple_ip_t));
  if (!new_tuple_chain->prv) {
    sw_log("%s:%d malloc() failed: %s", __FILE__, __LINE__, strerror(errno));
    return -1;
  }
  memcpy(new_tuple_chain->prv, tuple_chain->prv, sizeof(sw_tuple_ip_t));
#endif
  return sw_tuple_clone(&new_tuple_chain->next, tuple_chain->next);
}

static char *__pretty(sw_tuple_t *tuple_chain) {
  sw_tuple_ip_t *prv = tuple_chain->prv;
  char *b = sw_pretty_get_buffer();
  sprintf(b, "%s/%s", sw_pretty_ip(ntohl(prv->ip)), 
          sw_tuple_pretty(tuple_chain->next));
  return b;
}

static int __free(sw_tuple_t *tuple_chain) {
#ifdef USE_MEMMGR
    if (tuple_chain->prv)
    {
      sw_tuple_ip_t * tmp = (sw_tuple_ip_t *)tuple_chain->prv;
      if(tmp->mem_idx != 0)
      sw_memgmt_tuple_free_buf(tmp->mem_idx);
    }
#else
    if (tuple_chain->prv){
      free(tuple_chain->prv);
  }
#endif
  return sw_tuple_free(tuple_chain->next);
}

sw_tuple_handler_t ip_tuple_handler = {
  "IP",
  IPPROTO_IP,
  &__fetch,
  &__commit,
  &__hash,
  &__cmp,
  &__clone,
  &__pretty,
  &__free,
  &__hash_simpl__
};
