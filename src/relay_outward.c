...
volatile static int global_stats_relay_out[5];
...
static int __create(sw_socket_handler_t *self) {
  ...
  if (!ext_if) {
    ...
    close(((sw_relay_outward_t *)self->priv)->socket);
    free(self->priv);
    return -1;
  }

  if (sw_debug_flags & SW_DEBUG_RELAY)
    sw_log("%s:%d opening outer iface %s for input",
           __FILE__, __LINE__, ext_if);

  self->socket = sw_if_open_for_input(ext_if, 11, &self->ifindex);
  if (self->socket == -1) {
    close(((sw_relay_outward_t *)self->priv)->socket);
    free(self->priv);
    return -1;
...
static int __cconf_out(sw_socket_handler_t *self, int interface, int if_idx) {
  ...
  self->priv = malloc(sizeof(sw_relay_outward_t));
  if (!self->priv) {
 ...
 ((sw_relay_outward_t *)self->priv)->socket = interface;
  ((sw_relay_outward_t *)self->priv)->idx = if_idx;
#else
  ...
  if (sw_debug_flags & SW_DEBUG_RELAY)
    sw_log("%s:%d opening inner iface %s for output",
           __FILE__, __LINE__, int_if);
((sw_relay_outward_t *)self->priv)->socket = sw_if_open_for_output(int_if);
#endif
  if (((sw_relay_outward_t *)self->priv)->socket == -1) {
    free(self->priv);
    return -1;
  }
...
static int __destroy(sw_socket_handler_t *self) {
  ...
  if (sw_debug_flags & SW_DEBUG_RELAY)
    sw_log("%s:%d closing inner iface %s for output",
           __FILE__, __LINE__, int_if);

  if (((sw_relay_outward_t *)self->priv)->socket != -1) {
    sw_if_close(((sw_relay_outward_t *)self->priv)->socket);
  }

  ext_if = sw_config_get("outer-interface", "N/A");

  if (sw_debug_flags & SW_DEBUG_RELAY)
    sw_log("%s:%d closing outer iface %s for input",
           __FILE__, __LINE__, ext_if);
...
//  if(iph->protocol == SW_TUPLE_PROTO_DEFAULT)
//  if(!sw_check_ip(snat_ip_ns, htonl(iph->daddr)))
//  {
//      return 0;
 // };
  ...
  //pthread_rwlock_wrlock(mtx);
  //pthread_rwlock_rdlock(&global_filter_lock);
  //int fm_res = sw_filter_match(&rule, hsrc ? hsrc : src, hdst ? hdst : dst,
    //                           SW_FILTER_MATCH_OUTWARD);
...
spinlock_unlock(self->sp);
  ...
  //pthread_rwlock_unlock(&global_filter_lock);
  spinlock_unlock(self->sp);
  if (sw_gauge_drop_stat(self->cBuf,hdst ? hdst : dst, length, SW_GAUGE_DIR_OUT) == -1) {
      if(hsrc)sw_tuple_free(hsrc); if(hdst)sw_tuple_free(hdst);
    sw_tuple_free(src); sw_tuple_free(dst);
...
//  global_stats_relay_out[PACKETS_OUT]++;
//  global_stats_relay_out[BYTES_OUT] += length;
//  sw_log("pakets:%d, lenght:%d", global_stats_relay_out[PACKETS_OUT], global_stats_relay_out[BYTES_OUT]);
  if (shape_flag) {
    switch(delaytime = sw_shape_schedule(dst, length, rule->shape.rate,
                                         SW_SHAPE_OUT, NULL)) {
      case 0: /* send packet immediately */
        if (sw_if_send(((sw_relay_outward_t *)self->priv)->socket,
                       packet, length,0,((sw_relay_outward_t *)self->priv)->idx) == -1) {
          global_stats_relay_out[DROP_SHAPE_SEND_FAILED]++;
          break; /* Drop */
        }
        if (sw_debug_flags & (SW_DEBUG_RELAY | SW_DEBUG_SHAPE))
        ...
        break;
      case -1: /* error */
        global_stats_relay_out[DROP_SHAPE_SCHEDULE_ERROR]++;
        break; /* Drop */
      default: /* packet is placed into a queue */
        if (sw_memgmt_get_buf(&p_buf, length) == -1)
          break; /* Drop */
        memcpy(p_buf->buf, packet, length);
        if (sw_shape_delay(process_qnty/2, delaytime, p_buf, length) == -1) {
          global_stats_relay_out[DROP_SHAPE_DELAY_ERROR]++;
    ...
    sw_if_send(((sw_relay_outward_t *)self->priv)->socket, packet, length,0 ,((sw_relay_outward_t *)self->priv)->idx);
...
static int __send(sw_socket_handler_t *self, char *packet, int length) {
  return sw_if_send(((sw_relay_outward_t *)self->priv)->socket, packet, length,0,((sw_relay_outward_t *)self->priv)->idx);
}

int sw_relay_outward_stats_null() {
  global_stats_relay_out[DROP_SHAPE_SEND_FAILED] = 0;
  global_stats_relay_out[DROP_SHAPE_SCHEDULE_ERROR] = 0;
  global_stats_relay_out[DROP_SHAPE_DELAY_ERROR] = 0;
  global_stats_relay_out[PACKETS_OUT] = 0;
  global_stats_relay_out[BYTES_OUT] = 0;  
  return 0;
}

int sw_relay_outward_stats_print() {
  sw_log("%s:%d OUTWARD_drops: send %d, schedule %d, delay %d",
          __FILE__, __LINE__, global_stats_relay_out[DROP_SHAPE_SEND_FAILED],
          global_stats_relay_out[DROP_SHAPE_SCHEDULE_ERROR],
          global_stats_relay_out[DROP_SHAPE_DELAY_ERROR]);
/*  sw_log("%s:%d OUTWARD_stats: packets %d, bytes %d", __FILE__, __LINE__,
         global_stats_relay_out[PACKETS_OUT],global_stats_relay_out[BYTES_OUT]);*/
  return 0;
}

sw_socket_handler_t relay_outward = {
  "OUTWARD",
  0,
  0,
  0,
