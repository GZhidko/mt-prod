...
static int __filter_match(sw_filter_rule_t **rule, sw_tuple_t *tuple_chain_a,
  ...
  /* TCP flags */
  if ((*rule)->flags.mask) {
    if ((*rule)->flags.set != ((*rule)->flags.mask & prv_a->flags)) {
      if (sw_debug_flags & SW_DEBUG_FILTER)
        sw_log("%s:%d [%d] TCP flags %s != %s/%s mismatch",
               __FILE__, __LINE__, (*rule)->num,
               sw_pretty_flags(prv_a->flags),
               sw_pretty_flags((*rule)->flags.set), 
               sw_pretty_flags((*rule)->flags.mask));
      return 0;
    }
  }

  if (sw_debug_flags & SW_DEBUG_FILTER)
    sw_log("%s:%d [%d] matched", __FILE__, __LINE__, (*rule)->num);
...
sw_filter_match_handler_t tcp_filter_match_handler = {
  IPPROTO_TCP,
  &__filter_match
};
