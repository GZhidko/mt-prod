...
int sw_nat_src_create() {
  global_nat_src_prv_hash = lh_new(__prv_hash_cb, __prv_cmp_cb,0);
  if (!global_nat_src_prv_hash) {
    sw_log("%s:%d lh_new() failed: %s", __FILE__, __LINE__, strerror(errno));
    return -1;
  }

  global_nat_src_pub_hash = lh_new(__pub_hash_cb, __pub_cmp_cb,0);
  if (!global_nat_src_pub_hash) {
    sw_log("%s:%d lh_new() failed: %s", __FILE__, __LINE__, strerror(errno));
    return -1;
  }

  global_nat_src_seq_hash = lh_new(__seq_hash_cb, __seq_cmp_cb,0);
  if (!global_nat_src_seq_hash) {
    sw_log("%s:%d lh_new() failed: %s", __FILE__, __LINE__, strerror(errno));
    return -1;
  }
  ...
  sw_lhstat_new("snat-seq", global_nat_src_seq_hash);
...
int sw_nat_src_do(sw_tuple_t **pub, sw_tuple_t *prv) {
  ...
  if (r) {
     ...
     //sw_log("%s:%d  relation #%u %s [%s]", __FILE__, __LINE__,
       //      r->seq, sw_tuple_pretty(r->prv), sw_tuple_pretty(r->pub));
    //specMutexLock(&r->mtx);
    ...
    if (gst < r->seq &&
      ...
      spinlock_unlock(sp_ss);
      __expire_current_seq(&r->seq);
      //spinlock_lock(&sp_gst);
      //r->seq = global_seq_ticker;
      //spinlock_unlock(&sp_gst);
      lh_insert(global_nat_src_seq_hash, (void *)r,&sp_ss);
      //if (sw_debug_flags & SW_DEBUG_NAT_SRC)
      sw_log("%s:%d inserted relation #%u %s [%s]", __FILE__, __LINE__,
             r->seq, sw_tuple_pretty(r->prv), sw_tuple_pretty(r->pub));
      spinlock_unlock(sp_ss);
    } else {
      if (sw_debug_flags & SW_DEBUG_NAT_SRC)
   ...
   // spinlock_lock(&sp_gst);
    ...
    /*lh_delete(global_nat_src_prv_hash, (void *)r,&sp_sp);
    lh_delete(global_nat_src_pub_hash, (void *)r,&sp_spu);
    ri = (sw_nat_src_relation_t *)lh_delete(global_nat_src_seq_hash, (void *)r,&sp_ss);
    if(ri)
    {
        sw_tuple_free(ri->prv);
        sw_tuple_free(ri->pub);
        free(ri);
    }*/
...
int sw_nat_src_undo(sw_tuple_t **prv, sw_tuple_t *pub) {
  ...
  if (r) {
    ...
    spinlock_unlock(sp);
    r = (sw_nat_src_relation_t *)lh_retrieve(global_nat_src_prv_hash, (void *)r_clone,&sp);
    if(!r)
    {
        if (sw_debug_flags & SW_DEBUG_NAT_SRC)
    ...
    if (sw_debug_flags & SW_DEBUG_NAT_SRC)  
      sw_log("%s:%d existing relation #%u [%s] %s", __FILE__, __LINE__,
             r->seq, sw_tuple_pretty(r->pub), sw_tuple_pretty(r->prv));
